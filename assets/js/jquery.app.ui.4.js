...
$(document).ready(function () {
	...
	$("#instCheck input[type='checkbox']").change(function(){
		...
		toggleTextReg("txtInst","chkBoth",this);
	     ...
	     $("#area-32").change(function(){
                    $("#txtOpcion").slideToggle();
                });
    ...
    $("#btn_Send").click(function () {
		...
		var txtoth=$("#txtInst").val();
                var txtopcion=$("#txtOpcion").val();
                var mats=[];
		$(".areai input:checked").each(function(){
		...
		var sw=true;
		if(!val_txt(txtName)){
			msj="Especifica tu nombre";
			sw=false;
		}
		if(!val_txt(txtEmail)){
			msj="Especifica tu Email";
			sw=false;
		}
		if(!val_txt(type)){
			msj="Especifica si eres profesor, estudiante o padre de familia";
			sw=false;
		}
		if($(".error").is(":visible")){
		...
		if($('.chkgroup input:checked').length==0&&$("#txtOpcion").val()===""){
			
			sw=false;
			msj="Especifica que área(s) te interesan";
		}
		if($('#instCheck input:checked').length==0){
			sw=false;
			msj="Especifica si estas en el colegio, la universidad u otra institución";
		}
		if(sw===true){
			$("#form_contact_us").hide("slide");
			$("#load_gif").show("slide");
			console.log(uni);
                        var user_data={name:txtName,
				email:txtEmail,
				suger:txtMessage,
				type:type,
				col:col,
